(ns relativity_ediscovery.main
  "Kotodama WASM entrypoint for the Relativity_ediscovery clean-room actor (L4) — Clojure port.
  ...
  facts (`relativity_ediscovery.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "relativity_ediscovery")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"   :plural "matters"   :id-prefix "relativi_mat"
    :fields [:clientId :name :status :openedAt]      :required [:name :status]
    :coerce {}                       :refs {}}
   {:entity "Document"      :plural "documents"      :id-prefix "relativi_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"relativity_ediscovery.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "relativity_ediscovery-compat" :tier tier :entities entities} 200])
...
(defn start! [] :relativity_ediscovery-compat/ready)
